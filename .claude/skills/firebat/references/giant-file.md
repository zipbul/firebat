...
3. If the file resists decomposition (every function depends on every other), the tight coupling is the root cause. Address coupling first (break circular dependencies, extract shared types) before splitting the file.
