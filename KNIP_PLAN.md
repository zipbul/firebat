...
### Phase 0: gildash 0.15.1 Breaking Change 대응 + 즉시 적용 항목
...
| **Visitor API** — `Visitor`, `visitorKeys`, `VisitorObject` re-export. 기존 `visit`/`collectNodes` 삭제 | early-return 불가로 전면 교체 불가. 전체 순회만 하는 곳(indirection, temporal-coupling, barrel, unknown-proof 대부분)은 교체 가능. 함수 경계 차단이 필요한 곳(error-flow, ast-normalizer)은 `walkOxcTree` 유지. oxc-walker upstream에 feature request 예정 | 별도 PR |
...
| **buildLineOffsets/getLineColumn re-export** | `source-position.ts` (16줄) 삭제 가능 | 별도 PR |
...
| `ParsedFile` 중복 타입 | ~7줄 |
| `getNodeType`, `getNodeName` 래퍼 + 사용처 | ~20줄 정의 + 9개 파일 사용처 교체 |
...
| `function-span.ts` (14줄) | 삭제. `getSymbolsByFile()[].span` 사용 | Phase 1 |
| fan-in/out 수동 계산 (39줄) | gildash `getFanMetrics()` 확인 후 전환 | gildash API 존재 확인 후 별도 PR |
| barrel `checkCrossModuleReexport()` 패턴 B/C | gildash re-export 패턴 B/C 반영 완료. `collectImportBindings()` 57줄 삭제 가능. `collectImportLikes()`는 span 필요하므로 유지 | 별도 PR |
