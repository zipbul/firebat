코드 복잡도 메트릭	코드 분석 도구의 영역. 인덱싱/그래프와 무관	
JSDoc 커버리지	문서화 품질 메트릭. 린터의 영역	
레이어 아키텍처 검증	아키텍처 린팅 도구(dependency-cruiser 등)의 영역	
심볼 rename	IDE/리팩토링 도구의 영역. TS Language Server가 할 일	
프로젝트 통계 대시보드	"대시보드"는 UI 레이어. 집계 쿼리 API 정도는 가능하지만 대시보드 자체는 범위 밖
