...
**전체-인덱싱 전제 (판정 가능 조건)**: 판정은 파일 하나가 아니라 전체 프로그램의 import 그래프와 소비 관계(imports·re-exports·type-references·calls)로 한다. "사용처 0" 판정은 소비자가 그래프에 전부 있어야 참이므로 스캔 스코프가 부분이면 판정하지 않는다. 그중 심볼 소비 판정(dead-export·test-only-export·미사용 멤버)은 관계 인덱스가 불완전해도 보류한다(FP 방지) — 없는 관계를 근거로 W를 만들지 않는다. unused-file은 관계가 아니라 그래프 완전성과 진입점 해석에 의존한다. waste·indirection이 파일 안에서 닫히듯, dependencies는 완전한 그래프 안에서 닫힌다.
...
**대상 (11 kind, 6 계열)**:
1. **도달성** — `dead-export`: export 심볼인데 self 제외 외부 소비자가 0. 재노출 체인이 최종 소비자에 닿지 못하면 원본도 dead로 전파한다. `unused-file`로 보고된 파일의 export는 dead-export 후보에서 제외한다(파일 단위 보고가 우선 — 이중 보고 없음). 모듈이 전량 소비(namespace `*` import·`export *`)되면 그 모듈의 개별 export·멤버는 dead·member 판정을 하지 않는다. side-effect-only import(`import './mod'`)는 파일 도달성에만 기여하고 어떤 export도 소비하지 않는다(전량 소비 아님). `unused-file`: 진입점(패키지 진입점·설정·test)에서 도달 불가능한 non-test 파일 — 진입점이 하나도 없으면 보류. `test-only-export`: 소비자는 있으나 전부 test 경로라 prod 소비자가 0. `unused-enum-member`/`unused-ns-member`: 부모(enum/namespace)로의 멤버 호출이 최소 1개 관측되는데(부모 사용 증거) 이 멤버로의 호출은 없음 — 호출의 귀속은 닫힌 규칙으로 판정한다: 호출 관계의 대상이 부모 정의 모듈이거나, 호출한 파일이 그 부모 이름을 해당 모듈에서 import했고 호출 대상이 `Parent.` 접두이면 그 모듈로 귀속; 그 외 형태의 `Parent.` 접두 호출이 존재하면 귀속 불명이므로 보류. 부모 호출이 0이어도 보류.
2. **순환** — `circular-dependency`: 두 파일 이상이 직접·간접 import 순환을 형성.
3. **의존 선언 정합** — `unused-dependency`: dependencies·devDependencies에 선언했으나 import되지 않는 외부 패키지(peer/optional은 호스트가 제공하는 계약 선언이므로 미사용 후보가 아님). `unlisted-dependency`: import되나 선언(dependencies·devDependencies·peerDependencies·optionalDependencies 중 어느 필드든) 없는 외부 패키지.
...
**K 예시 (망라 아님)**: 재노출로 최종 소비자에 닿는 export, test·prod 양쪽에서 쓰이는 export, namespace `*` import·`export *`로 전 심볼 소비되는 모듈의 개별 export·멤버, side-effect-only import로 도달만 하는 파일(unused-file 아님), builtin(`node:`/`bun:`)·self 패키지·명시적 ignore 대상·`@types/X`(base 패키지 사용 또는 base가 builtin일 시)·package.json scripts에서 binary로 실행되는 선언 dep, peer/optional로 선언된 외부 패키지, external import(내부 참조 아님), 동명이지만 다른 원본을 가리키는 export, 원본 해석이 실패한 동명 export(보수적 비병합), 부모 호출이 관측되지 않아 보류된 멤버, 진입점 미정의 시 unused-file, 레이어 밖 파일의 엣지, 관계 불완전으로 보류된 dead-export.
