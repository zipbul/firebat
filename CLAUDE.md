...
함수·클래스·타입 등 결정을 담은 구문이 **바인딩·리터럴·타입 치환만으로 정규형이 일치**해 두 곳 이상에 복제되어, 한 결정의 **단일 변경지점이 깨진** 구조를 보고한다.
...
**구조 일치 예외 (역할로 판정, 의도와 구분)**: 정규형이 같아도 그 구문이 **결정을 담지 않는 골격**(외부가 형태를 강제하는 단순 위임·본문 없는 overload 시그니처·프로토콜 강제 구조)이면 K다. 드리프트할 결정이 없어 같이 바뀔 의무가 안 생기기 때문. 이는 구문의 역할을 형태로 판정하는 *닫힌 규칙*이지 "같은 의도로 복제했나"라는 주관 판단이 아니다 — 결정(제어흐름·계산·계약)을 담은 구문이면 정규형이 같은 순간 **항상 W**다.

**판정 절차**: 두 구문의 바인딩(식별자·지역변수·타입파라미터)·리터럴·타입인자를 placeholder로 치환한 정규형이 일치하고 둘 다 결정을 담으면 W, 정규형이 어긋나거나 결정 없는 골격이면 K.

**대상**: 결정을 담는 선언 단위 — 함수·메서드·클래스, 계약을 담는 타입·인터페이스 정의. 데이터라도 그 자체가 규칙(매핑·룩업 테이블)이면 대상. OXC AST 정규형으로 판정.
