...
모든 detector의 판정 근거는 **검증 가능한 사실**만 허용한다: 언어·런타임 명세(ECMAScript/TS 의미론, AST 노드 구조), 스코프 해석으로 닫힌 바인딩·타입 identity, 선언된 계약(package.json 필드·`node:`/`bun:` prefix·tsconfig), 사용자 명시 설정(glob·allowlist).

**추측값 금지**: 추측값이란 판정의 근거인데 참인지 검증할 수 없는 것 — 소비자가 임의로 짓는 이름·파일명·경로·표기 관습에서 역할이나 의미를 짐작한 값이다. 추측은 틀리는 순간 거짓 W(노이즈) 또는 부당 K(suppression)를 만들므로 판정 근거로 쓰지 않는다.

**닫히지 않으면 보류**: 사실로 판정을 닫을 수 없으면 W를 만들지 않고 보류한다(FN 방향). 명세가 정의한 이름은 대상의 identity(바인딩 해석·타입 증명)가 확인될 때만 명세 사실로 취급하며, identity 미확인 상태의 이름 매칭은 K방향(finding을 만들지 않는 쪽)으로만 작용할 수 있다.
...
**대상**: 결정을 담는 두 종류의 단위. ① **선언** — 함수·메서드·클래스, 계약을 담는 타입·인터페이스 정의, 규칙을 담는 데이터 선언(매핑·룩업 테이블). 계약은 선언 구문과 무관하게 구조로 비교한다 — 같은 멤버 구조가 interface와 type alias 양쪽에 있으면 같은 계약의 중복이다. **행위-보유 명명 선언(함수·메서드·클래스·데이터 선언)은 크기 floor가 없다** — 작은 중복 함수도 주소 지정 가능한 변경지점이므로 잡는다(false negative 방지). ② **함수 내부 연속 문장열(statement run)** — 함수 본문에 복제된 문장 덩어리. OXC AST 정규형으로 판정한다.

**결정-존재 floor (익명 인라인 표현식 + 계약 타입선언)**: 결정-존재 floor(`minSize`)는 문장열뿐 아니라 **익명 함수 표현식**(arrow / 이름 없는 function expression)과 **계약 타입선언**(interface·type alias)에도 적용한다. 익명 인라인 람다는 명명된 변경지점이 아니라 인라인 코드(문장열의 인라인 등가물)이고, 계약 타입선언은 행위 없는 순수 구조라 floor 미만의 초소형 shape(`{line;column}` 등)는 독립 모듈이 우연히 수렴하는 보편 어휘다 — 사소한 람다의 인터페이스 등가물(이름은 정규형에서 alpha-renaming으로 치환되므로 명명 여부가 정보량을 만들지 않는다; 우연 일치 확률은 구조 크기에 따라 지수적으로 감소하는 정보량 원리의 노드 수 근사). 정규형 노드 수가 floor 미만이면 드리프트할 결정을 담기엔 너무 작아 K다. 형태로 판정하는 닫힌 규칙(노드 유형 + 노드 수)이며 매칭 자체는 정규형 완전 일치(이진·닫힘)다 — 우연히 같은 사소한 람다(`(a,b)=>a-b` 비교자, `s=>s.length>0` 술어, `n=>f(n,k)` 투영)와 초소형 보편 shape(`interface P {line:number; column:number}`) 등 독립 결정의 동형을 zero-FP로 거른다. **대가는 floor 미만의 진짜 복붙 소형 계약을 놓치는 FN**이며, literal-variant 비탐지와 같은 방향의 의도된 trade다. **행위-보유 명명 선언은 이 floor 비대상**(작아도 변경지점).
...
- **결정-존재 floor 미만의 익명 인라인 표현식**(arrow / 이름 없는 function expression)과 **계약 타입선언**(interface·type alias): 너무 작아 결정을 담지 못함(보편 shape의 우연한 동형) — 행위-보유 명명 선언은 비대상 floor (위 닫힌 규칙)
- export 표면의 cross-module 중복 (dependencies 영역)

**K 예시 (망라 아님)**: 외부강제 골격(단순 위임·overload 시그니처·프레임워크 등록 형태), 정규형이 어긋나는 유사 코드, 형태만 같고 결정을 담지 않는 데이터 구조, 결정-존재 floor 미만의 익명 인라인 람다(사소한 비교자·술어·투영), floor 미만의 초소형 보편 shape 계약(`{line;column}` 등 — 독립 모듈의 우연한 수렴, 위치·경로 기반 예외 없이 floor로만 판정).
...
전체 import 그래프에서 **모듈 간 의존 계약이 실현되지 않거나 깨진** 구조를 보고한다.

**모듈 간 의존 계약**: export·파일이 실제 소비 흐름에 도달하는지, 내부 참조가 파일로 해결되는지, 외부 패키지의 선언과 import가 일치하는지, import 엣지가 허용된 레이어 안에 있는지, export 표면이 같은 원본을 중복 노출하지 않는지. 코드 스타일·저자 의도·이름을 통한 가독성·재노출 구문 취향은 포함하지 않는다.

**전체-인덱싱 전제 (판정 가능 조건)**: 판정은 파일 하나가 아니라 전체 프로그램의 import 그래프와 소비 관계(imports·re-exports·type-references·calls)로 한다. "사용처 0" 판정은 소비자가 그래프에 전부 있어야 참이므로 스캔 스코프가 부분이면 판정하지 않는다. 그중 심볼 소비 판정(dead-export·미사용 멤버)은 관계 인덱스가 불완전해도 보류한다(FP 방지) — 없는 관계를 근거로 W를 만들지 않는다. unused-file은 관계가 아니라 그래프 완전성과 진입점 해석에 의존한다. waste·indirection이 파일 안에서 닫히듯, dependencies는 완전한 그래프 안에서 닫힌다.

**판정 절차**: import 그래프·export 심볼·소비 관계·순환 경로·원본 해석을 전체 프로그램에서 얻어(gildash 의미 그래프 — OXC AST 아님), 각 kind를 그래프·집합·glob의 닫힌 규칙으로 이진 판정한다(유사도·임계 없음). 경로는 프로젝트 루트 기준으로 정규화한다.

**대상 (10 kind, 6 계열)**:
1. **도달성** — `dead-export`: export 심볼인데 self 제외 외부 소비자가 0(소비자가 test 파일이든 뭐든 1개라도 있으면 dead 아님 — "test 소비는 실사용 아님"은 파일명으로 닫히지 않는 추측이라 판정하지 않는다). 재노출 체인이 최종 소비자에 닿지 못하면 원본도 dead로 전파한다. `unused-file`로 보고된 파일의 export는 dead-export 후보에서 제외한다(파일 단위 보고가 우선 — 이중 보고 없음). **진입점 모듈(패키지 진입점 ∪ 사용자 `entry` glob)의 export는 dead-export 비대상** — 그 소비자는 정의상 그래프 밖(러너·문자열 로드 도구·외부 패키지)이므로 외부 계약이다. 모듈이 전량 소비(namespace `*` import·`export *`·dynamic `import()` — 런타임에 모듈 네임스페이스 객체 전체를 받으므로, gildash `meta.isDynamic`이 사실 원천)되면 그 모듈의 개별 export·멤버는 dead·member 판정을 하지 않는다. side-effect-only import(`import './mod'`)는 파일 도달성에만 기여하고 어떤 export도 소비하지 않는다(전량 소비 아님). `unused-file`: 진입점에서 도달 불가능한 파일. **opt-in** — 사용자가 `entry` glob을 선언했을 때만 보고(그 선언이 "진입점 집합이 완전하다"는 사실). 미선언 시 완전성이 증명 안 되므로 보류(FN 방향, 파일명 추측으로 진입점을 짓지 않는다 — 폭발 없음). 진입점 = 패키지 진입점(package.json main/module/exports/bin, 자동 사실) ∪ 사용자 선언 `entry` glob(사실). **파일명 관습 seed 없음**(추측값 제거). 사용자 `ignore` glob에 매칭된 파일은 unused-file 보고에서 제외한다. 도달성이 계산 가능할 때(진입점 존재) 미도달 파일의 export는 dead-export에서 제외한다(파일 단위 보고 우선 — 이중 보고 없음). `unused-enum-member`/`unused-ns-member`: 부모(enum/namespace)로의 멤버 호출이 최소 1개 관측되는데(부모 사용 증거) 이 멤버로의 호출은 없음 — 호출의 귀속은 닫힌 규칙으로 판정한다: 호출 관계의 대상이 부모 정의 모듈이거나, 호출한 파일이 그 부모 이름을 해당 모듈에서 import했고 호출 대상이 `Parent.` 접두이면 그 모듈로 귀속; 그 외 형태의 `Parent.` 접두 호출이 존재하면 귀속 불명이므로 보류. 부모 호출이 0이어도 보류.
2. **순환** — `circular-dependency`: 두 파일 이상이 직접·간접 import 순환을 형성.
3. **의존 선언 정합** — `unused-dependency`: dependencies·devDependencies에 선언했으나 (TS import 그래프에서) import되지 않는 외부 패키지(peer/optional은 호스트가 제공하는 계약 선언이므로 미사용 후보가 아님). 보류 규칙: ① `@types/*`는 전량 보류(deadness가 tsconfig `types`/`typeRoots`·`extends`·`references`·triple-slash 병합 없이 안 닫힘), ② 설치된 `bin`-보유 패키지는 보류(script·git hook·`bunx`·수동 실행 등 정적 그래프 밖 호출을 증명 불가 — 쉘 파싱 대신 dep의 package.json `bin` 필드를 사실로 읽음, `typescript`→`tsc`처럼 바이너리명≠패키지명 포함). manifest 미읽힘(pnpm 비평면·Yarn PnP·rootAbs 상위 hoist)은 `unknown`→**보류**(install-state 부재를 W 근거로 쓰지 않음). import도 안 되고 `bin`도 없는 확정 라이브러리만 보고. `unlisted-dependency`: import되나 선언(dependencies·devDependencies·peerDependencies·optionalDependencies 중 어느 필드든) 없는 외부 패키지.
4. **해결** — `unresolved-import`: 내부 참조(import·재노출, type 포함)가 대상 파일로 해결되지 않음.
5. **재노출 표면** — `duplicate-export`: **독립적으로 정의된** 동명 export가 같은 원본으로 해석되어 2개 이상의 표면에 중복 노출(재노출 구문으로만 노출된 표면은 비교 대상이 아님 — 재노출 표면 구성은 barrel 영역).
6. **레이어 계약** — `layer-violation`: 서로 다른 레이어 glob에 매칭되는 두 파일 사이의 import가 허용 목록에 없음 — 레이어 미설정이면 전체 비활성.

**비대상 (다른 detector 영역 / 판정 밖)**:
- 부분 스캔·불완전 관계 인덱스 — 전체-인덱싱 전제 위반이므로 보류
- 재노출 구문 형태(`export *`·deep-import·barrel 구성 등 스타일) — barrel 영역. dependencies는 재노출을 스타일이 아니라 소비 흐름으로만 본다
- 무변형 위임 간접층(thin-wrapper·forwarding-chain) — indirection 영역. 위임은 그래프 엣지일 뿐이다
- 코드 본문의 정규형 중복 — duplicates 영역. duplicate-export는 export 표면이지 구현 본문이 아니다
- fanIn/fanOut/instability metric 기반 hotspot(god-module 등) — coupling 영역. dependencies는 raw metric 공급자다
- 비-TS 파일(.js 등)에서의 소비 — firebat은 TS 전용이므로 판정 밖(TS import 기준으로만 판정)
- 런타임 도달성·동적 로딩 — 정적 import 그래프 밖
- enum/namespace 멤버의 값-참조 — 호출로 관측되지 않는 사용은 판정 밖(보류, FN 방향)

**K 예시 (망라 아님)**: 재노출로 최종 소비자에 닿는 export, 소비자가 하나라도 있는 export(test 파일 소비자만 있어도 dead 아님 — 파일명으로 test/prod를 가르지 않음), 사용자 `entry` 또는 package.json 진입점에서 도달하는 파일, 사용자 `ignore` glob 대상 파일(unused-file 아님), 사용자가 `entry` 미선언 시 보류된 unused-file, namespace `*` import·`export *`로 전 심볼 소비되는 모듈의 개별 export·멤버, side-effect-only import로 도달만 하는 파일(unused-file 아님), builtin(`node:`/`bun:`)·self 패키지·명시적 ignore 대상·**모든 `@types/*`**(deadness가 tsconfig `types`/`typeRoots`·`extends`·`references`·triple-slash 병합 없이 안 닫히고 ambient 전역은 import 없이 소비되므로 전량 보류 — FN), **설치된 `bin`-보유 패키지**(script·hook·`bunx`·수동 실행이 정적 그래프 밖이라 비사용 증명 불가 — dep의 package.json `bin` 필드가 사실 원천, `typescript`→`tsc` 포함), **manifest 미읽힘 dep**(pnpm 비평면·Yarn PnP·상위 hoist → `unknown` → 보류, install-state 부재는 W 근거 아님), peer/optional로 선언된 외부 패키지, external import(내부 참조 아님), 동명이지만 다른 원본을 가리키는 export, 원본 해석이 실패한 동명 export(보수적 비병합), 부모 호출이 관측되지 않아 보류된 멤버, 진입점 미정의 시 unused-file, 레이어 밖 파일의 엣지, 관계 불완전으로 보류된 dead-export.
