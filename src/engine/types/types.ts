...
// IntegerCFG is defined in ../cfg/cfg.ts but crosses through the cfg surface here
// (type-only edge — no runtime cycle) rather than reaching past it, since cfg/cfg.ts
// itself is not this module's own subtree.
import type { IntegerCFG } from '../cfg';
// NodeId is co-located in this same directory (cfg-types.ts) — a shared leaf pulled
// in from ../cfg/cfg-types.ts to break the historical types<->cfg barrel cycle.
