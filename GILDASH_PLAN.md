...
**참고**: dependency-cruiser에 `engine/` 레이어 격리 규칙이 별도로 없으므로 `bun run deps`는 이 변경을 검증하지 않음. 타입 전용 import (`import type`)이므로 런타임 영향 없음을 코드 리뷰로 확인.
...
2. 코드 리뷰로 `import type` 확인 (dependency-cruiser에 engine 격리 규칙 없음)
...
1. gildash가 `lib.es5.d.ts` (TypeScript 내장) 파일을 semantic 대상에 포함하는지 확인
2. `searchSymbols({ text: 'PromiseLike', exact: true })`로 `PromiseLike` 위치 런타임 탐색 가능 여부 확인
3. `getImplementations('Error', ...)` 동작 확인
4. 확인 결과에 따라 P5-1, P5-5 실행 여부 결정
...
3. P4-1: `isTypeAssignableToAt`로 `isPromiseLike` 교체 (Phase 3 검증 통과 시)
4. P4-2: `getImplementations`로 Error 서브클래스 자동 탐색 (Phase 3 검증 통과 시)
5. P6-4: indirection — semantic wrapper detection 강화
6. P4-2: `resolveSymbol`로 indirection re-export 체인 분석 강화
